...
internal sealed class BlobStorageConfiguratorService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (containerNames.Contains(StorageService.PublicAuthenticationContainerName, StringComparer.OrdinalIgnoreCase))
        {
        ...
        var createResult = await _blobServiceClient
            .CreateBlobContainerAsync(StorageService.PublicAuthenticationContainerName, cancellationToken: cancellationToken);

        if (createResult.GetRawResponse().Status != 201)
        {
            _logger.LogError("Failed to create container {ContainerName}. Status code: {StatusCode}", StorageService.PublicAuthenticationContainerName, createResult.GetRawResponse().Status);
            return;
        }

        var policySetupResult = _blobServiceClient
            .GetBlobContainerClient(StorageService.PublicAuthenticationContainerName)
        ...
        {
            _logger.LogError("Failed to setup policy for {ContainerName}. Status code: {StatusCode}", StorageService.PublicAuthenticationContainerName, policySetupResult.GetRawResponse().Status);
            return;
        }
